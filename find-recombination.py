...
callHandle.write('readname\tcontig\tmap-position\treference-flag\talternate-flag\trecombination-flag\treference-counts\talt-counts\terror-counts\tcall-list\n')
					...
					if len(call_list)>20:
						...
						if len(firstset)==1 and len(lastset)==1:#ensure no switching in the first or last 10
							if firstset!=lastset:#first and last do not match
								recombFlag=1
								recombcounter+=1
					callHandle.write('%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n'%(readname,mapcontig,mappos,refFlag,altFlag,recombFlag,refcounter,altcounter,errorcounter,call_list))
